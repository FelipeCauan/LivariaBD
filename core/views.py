...
def lista_livros(request):
    # O select_related realiza o SQL JOIN entre Livro e Prateleira
    livros = Livro.objects.select_related('prateleira').all() 
    return render(request, 'core/livros.html', {'livros': livros})

def dashboard(request):
    # Relatório: Estoque Baixo (WHERE qtdEstoque <= 5)
    estoque_baixo = Livro.objects.filter(qtdEstoque__lte=5) #

    # Relatório: Top 5 Mais Vendidos (JOIN + COUNT + LIMIT)
    # Mudamos de 'venda' para 'venda_set' para o Django encontrar a relação inversa
...
def realizar_venda(request):
    ...
    context = {
        'livros': Livro.objects.filter(qtdEstoque__gt=0), # Apenas livros com estoque
